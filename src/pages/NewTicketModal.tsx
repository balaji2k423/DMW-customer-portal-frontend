...
import {
  ...
  ChevronDown, Building2, Tag, Flag, FolderOpen,
...
const inputCls =
  "w-full rounded-xl border border-border bg-background px-4 py-2.5 text-[14px] outline-none transition-all placeholder:text-muted-foreground/30 focus:border-orange-400 focus:ring-2 focus:ring-orange-400/15";
...
function fileIcon(filename: string) {
  ...
  if (["jpg","jpeg","png","gif","webp","svg"].includes(ext))
    return <ImageIcon className="h-4 w-4 text-muted-foreground/50" strokeWidth={1.5} />;
...
export default function NewTicketModal({ open, onClose, onCreated, projects = [] }: Props) {
  ...
  const [form, setForm] = useState<TicketForm>({
    subject: "", description: "", priority: "medium", category: "hardware", project: "",
  });
  ...
  const handleClose = () => {
    setForm({ subject: "", description: "", priority: "medium", category: "hardware", project: "" });
    setAttachments([]);
    setErr(null);
    setSuccess(false);
    onClose();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const ticket = await ticketsService.create({
        ...
        // FIX: category is always a string now (default "hardware"), never undefined
      ...
      setSuccess(true);
      setTimeout(() => { onCreated(); handleClose(); }, 1200);
    } catch (err: any) {
      ...
      <div className="w-full max-w-lg overflow-hidden rounded-2xl border border-border bg-card shadow-2xl">
        ...
        {success && (
          <div className="flex flex-col items-center justify-center gap-4 py-20 px-8 text-center">
            <div className="flex h-16 w-16 items-center justify-center rounded-full bg-emerald-50 dark:bg-emerald-500/10 border-2 border-emerald-200 dark:border-emerald-500/25">
              <CheckCircle2 className="h-8 w-8 text-emerald-500" strokeWidth={2} />
            </div>
            <div>
              <p className="text-[17px] font-bold">Ticket raised!</p>
        ...
        {!success && (
          <form onSubmit={handleSubmit} className="space-y-4 p-6 max-h-[80vh] overflow-y-auto">
            ...
            <div>
              <label className="mb-1.5 block text-[12px] font-bold uppercase tracking-[.12em] text-muted-foreground/60">
                Subject <span className="text-rose-500 normal-case font-normal tracking-normal">*</span>
              </label>
              <input
                className={inputCls}
                required
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <div className="relative">
                  ...
                  <select
                    className={cn(inputCls, "appearance-none pl-10 pr-10")}
                    value={form.priority}
            ...
            <div>
              <label className="mb-1.5 block text-[12px] font-bold uppercase tracking-[.12em] text-muted-foreground/60">
                Project{" "}
                {projects.length === 0 && (
                  <span className="text-rose-500 normal-case font-normal tracking-normal">— no projects available</span>
                )}
              </label>
              <div className="relative">
            ...
            <div className="flex gap-3 border-t border-border pt-4">
              ...
              <button
                ...
                className="flex flex-1 items-center justify-center gap-2 rounded-xl py-2.5 text-[14px] font-semibold text-white disabled:opacity-60 transition-all hover:opacity-90 active:scale-95"
                style={{ background: BRAND }}
              >
